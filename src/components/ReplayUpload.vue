...
<template>
    <FileUpload multiple :max-file-size="MAX_FILE_SIZE" :file-limit="MAX_FILE_COUNT" custom-upload @uploader="replayUploader($event)">
        <template #empty>
            <p>Drag and drop files to here to upload.</p>
        </template>
        <template #content="{ files, uploadedFiles, removeFileCallback, removeUploadedFileCallback }">
            <div v-if="files.length > 0">
                <div class="flex flex-row items-center space-x-12 px-5 py-1" v-for="(file, index) in files" :key="file.name + index">
                    <div>{{ file.name }}</div>
                    <div>{{ (file.size / 1024).toFixed(2) }} KB</div>
                    <Badge class="px-3" value="Pending" severity="warning"/>
                    <Button icon="pi pi-times" @click="removeFileCallback(index)" outlined rounded severity="danger"></Button>
                </div>
            </div>
            <div v-if="uploadedFiles.length > 0">
                <div v-for="(file, index) in uploadedFiles" :key="file.name + index">
                    ...
                    <Button icon="pi pi-times" @click="removeUploadedFileCallback(index)" outlined rounded severity="danger"></Button>
                </div>
            </div>
        </template>
    </FileUpload>
